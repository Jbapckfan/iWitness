...
struct NASSetupView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        Text("Connect to any WebDAV-compatible storage.")
                        Text("For Hetzner Storage Box:")
                            .fontWeight(.bold)
                        Text("• URL: https://<username>.your-storagebox.de")
                        Text("• User: u123456")
                        Text("• Pass: (Your WebDAV password)")
                    }
                }
                ...
                Section {
                    ...
                    .disabled(nasURL.isEmpty || username.isEmpty || password.isEmpty || isTesting)
            ...
            .navigationTitle("Offsite Storage")
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button("Cancel") {
                        dismiss()
    ...
    private func testConnection() {
        ...
        Task {
            ...
            do {
                let success = try await WebDAVService.shared.validateConnection(url: url, credentials: credentials)
                if success {
                    testResult = "Success! Connected to Storage Box."
                } else {
                    testResult = "Connection failed. Check settings."
                }
            } catch {
                ...
                if vaultManager.isAuthenticated {
                    if vaultManager.vaultFiles.isEmpty {
                        VStack(spacing: 20) {
                            ...
                            Text("Recordings saved here are hidden from your localized Photos library and iCloud.")
                                .font(.caption)
