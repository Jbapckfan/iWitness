...
enum AWSV4Signer {
    ...
    /// - Returns: A new URLRequest with Authorization and x-amz-date headers, or the original request on failure
    static func sign(
        request: URLRequest,
        keys: SigningKeys,
        ...
        // 3. Canonical Request
        // AWS V4 signing requires exact string formatting — no leading/trailing whitespace
        ...
        guard let canonicalRequestData = canonicalRequest.data(using: .utf8) else {
            debugLog("[AWSV4Signer] Failed to encode canonical request")
            return request
        }
        ...
        guard let secretKeyData = "AWS4\(keys.secretKey)".data(using: .utf8) else {
            debugLog("[AWSV4Signer] Failed to encode secret key")
            return request
        }

        let dateKey = hmac(string: dateStamp, key: secretKeyData)
        ...
        let signature = hmac(string: stringToSign, key: signingKey)
            .compactMap { String(format: "%02x", $0) }.joined()
    ...
    private static func hmac(string: String, key: Data) -> Data {
        ...
        // UTF-8 encoding of ASCII-compatible signing strings never fails
        let data = string.data(using: .utf8) ?? Data()
        let code = HMAC<SHA256>.authenticationCode(for: data, using: keySym)
        return Data(code)
    }
}
